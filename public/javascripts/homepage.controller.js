(function () {
	angular.module("app", []).controller("mainController", function ($scope, $http) {
		...
		$scope.getAllEmployees = function () {
			$http.get("/getAllEmployees").success(function (param1, param2, param3) {
				$scope.employees = param1;
			})
		};
		...
		$scope.createEmployeeDetails = function () {
			...
			if (($scope.empObj.name != "") && pattern.test($scope.empObj.email) && ($scope.empObj.DOB != "") && ($scope.empObj.dept != "") && ($scope.empObj.gender != "")) {
				...
				$http({ url: "/createAEmployee", method: "POST", data: $scope.empObj }).success(function (param1, param2, param3) {
					$scope.employees.push(param1);
					$scope.empObj = {
						name: "",
						email: "",
						dob: "",
		...
		//deleting employee from collection
		$scope.deleteEmployeeDetails = function (employee) {
			$http({ url: "/deleteAEmployee/" + employee._id, method: "DELETE", data: employee }).success(function (param1, param2, param3) {
				$scope.getAllEmployees();
			}).error(function (err) {
